...
function edit__html(pages, pdf__name, format, cache_data) {
    ...
    if  (pages > 9) {
        for (c; c<pages; c++) {
            ...
            <img src="${page_count}.${format}?=${cache_data}" alt="" style="width: 100%; max-width: none;"><br>`);
            i++;
        }   
    }
        ...
        for (c; c<pages; c++) {
            document.querySelector("body").innerHTML += (`
            <img src="${i}.${format}?=${cache_data}" alt="" style="width: 100%; max-width: none;"><br>`);
            i++;
        }        
    }
    create__html();                 
...
//Realiza a escrita do html
function create__html() {
    fs.writeFile("./dist/index.html", document.documentElement.innerHTML, function(error) {
        console.log(document.documentElement.innerHTML)
        if (error) throw error;
    });
}
...
renomear(_terminal__arg)
>>>>>>> 8cae729 (corrige pasta de saide e nome das imagens):convert.js
